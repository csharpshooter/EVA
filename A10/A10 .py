...
saved_data, epoch, model_state_dict, optimizer_state_dict, train_losses, train_acc, test_losses, test_acc            , test_losses, lr_data, class_correct, class_total= utils.Utils.loadmodel(path="savedmodels/finalmodelwithdata.pt")
print(epoch)
...
plotdata.PlotData.plottesttraingraph(train_losses=train_losses,train_acc=train_acc,test_losses=test_losses,
                                     test_acc=test_acc,lr_data=lr_data, plotonsamegraph=True,epochs=epoch,doProcessArray=True)
...
gradcamimg,prediction = gradcamhelper.dogradcam(model=model,image=inputs[8],device=device,classes=classes)
